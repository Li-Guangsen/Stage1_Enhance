...
- 不用 502/496、2770 或历史 168 诊断替代当前 MyEdge 166 complete-case downstream validation。
...
## 3.1 MyEdge 166 Complete-Case Screening Baseline

MyEdge raw 目录仍有 168 张图，但当前主筛选口径统一为 166 complete-case：排除 `chazhuang.3.jpg` 与 `chazhuang.6.jpg`。后续 Stage1 candidate 的增强指标筛选和 fixed-detector downstream validation 都必须使用同一 166 stem set。
...
- EAAI-aligned v2 输出：`experiments/myedge168_compare9_rerun_20260527/metrics/enhancement_metrics_eaai_aligned_v2`
- 主表：`mean_metrics_9method_complete_case_166.md`，使用 `raw + 9` 口径中的 9 个增强方法共同 166 张 complete-case；`chazhuang.3.jpg` 与 `chazhuang.6.jpg` 被所有方法统一排除。
- 附表：`mean_metrics_8method_no_wwpf_168.md`，排除 WWPF 后覆盖完整 168 张。
- 新增指标：`SSEQ` 和 raw-enhanced `SIFT_MATCH_RATIO`。

该表不是 downstream 边缘检测结果；本轮未运行 DiffusionEdge/MSFI、`eval.py` 或 `show.py`。`HLRP` / `Histoformer` 继续标记为 high-noise diagnostic reference，不纳入主筛选结论；`WWPF` 标记为 incomplete 166/168。

## 4. Downstream Diagnostic

历史 downstream 事实限定在 MyEdge 168 张带 GT split、fixed MSFI 50k 和 fixed DiffusionEdge baseline 50k；后续新增 candidate 的主验证口径改为 MyEdge 166 complete-case。
