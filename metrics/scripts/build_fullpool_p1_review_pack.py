...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a P1 visual review pack for full_algae_dewatermark_v1 manual review. "
    ...
    parser.add_argument("--output-dir", default=str(DEFAULT_REVIEW_DIR / "p1_review_pack"))
...
def near_duplicate_suggestion(row: Dict[str, str]) -> Tuple[str, str]:
    ...
    same_file = evidence_value(evidence, "same_file_sha256") == "True"
    same_gray = evidence_value(evidence, "same_gray_image_sha256") == "True"
    if same_file or same_gray:
        if same_top:
            return (
                "mark_duplicate",
                "P1 pair has zero pHash/dHash distance and exact file or gray-image hash equality within the same top-level folder.",
            )
        return (
            "keep_but_split_guard",
            "P1 pair has zero pHash/dHash distance and exact file or gray-image hash equality across folders; keep until taxonomy is checked, but enforce leakage guard.",
        )
    if same_top:
        return (
            "needs_manual_visual_check",
            "P1 pair has zero pHash/dHash distance within one folder but file hashes differ; visually confirm whether it is a duplicate, repeated acquisition, or valid variant.",
        )
    return (
        "keep_but_split_guard",
        "P1 pair has zero pHash/dHash distance across folders; preserve until taxonomy is checked, but enforce split leakage guard.",
    )


def quality_suggestion(row: Dict[str, str]) -> Tuple[str, str]:
    flags = set(parse_flags(row.get("evidence_summary", "")))
    if "low_contrast" in flags or "low_laplacian_sharpness" in flags or "low_tenengrad_edges" in flags:
        return (
            "subset_label_only",
            "P1 quality outlier likely captures a degradation mode relevant to weak-boundary evaluation; label it for stratified analysis before excluding.",
        )
    return (
        "needs_manual_visual_check",
        "P1 quality outlier requires visual confirmation before deciding whether it is a valid degradation sample, failure case, or clean-pool exclusion.",
    )
...
def write_index_md(path: Path, summary: Dict[str, object], outputs: Dict[str, object]) -> None:
    lines = [
        ...
        "## Summary",
        "",
        f"- Near duplicate P1 rows: `{summary['near_duplicate_pair_rows']}`",
        f"- Quality outlier P1 rows: `{summary['quality_outlier_rows']}`",
        f"- Total P1 rows: `{summary['total_p1_rows']}`",
...
def main() -> None:
    ...
    near_rows = [row for row in read_tsv(Path(args.near_review)) if row.get("priority") == "P1"]
    quality_rows = [row for row in read_tsv(Path(args.quality_review)) if row.get("priority") == "P1"]
    ...
    recommendations_path = output_dir / "p1_review_recommendations.tsv"
    ...
    near_sheet_path = output_dir / "p1_near_duplicate_contact_sheet.png"
    ...
    for index, paths in enumerate(chunked(quality_preview_paths, args.quality_contact_sheet_chunk_size), start=1):
        sheet_path = output_dir / f"p1_quality_contact_sheet_{index:03d}.png"
    ...
    summary = {
        ...
        "total_p1_rows": len(recommendations),
    ...
    summary_json_path = output_dir / "p1_review_pack_summary.json"
    summary_md_path = output_dir / "p1_review_pack_summary.md"
