...
## 3.1 MyEdge 166 主筛选口径

MyEdge raw 目录仍有 168 张图，但当前主筛选口径已经统一为 166 complete-case：排除 `chazhuang.3.jpg` 与 `chazhuang.6.jpg`。后续 Stage1 candidate 的增强指标筛选和 fixed-detector downstream validation 都必须使用这同一批 166 张；历史 168 结果只作为历史诊断证据。
...
- v2 指标输出：`experiments/myedge168_compare9_rerun_20260527/metrics/enhancement_metrics_eaai_aligned_v2`
- 主表：`mean_metrics_9method_complete_case_166.md`，覆盖 `Ours + 8` 个外部方法的 166 complete-case，并统一排除 `chazhuang.3.jpg` 与 `chazhuang.6.jpg`。
- 附表：`mean_metrics_8method_no_wwpf_168.md`，排除 WWPF 后覆盖 168 张完整 MyEdge split。
- 新增指标：`SSEQ` 与 raw-enhanced `SIFT_MATCH_RATIO`。
- `HLRP` / `Histoformer` 只作为 high-noise diagnostic reference，不纳入主筛选结论。

该基线只用于后续 Stage1 candidate 的增强指标第一道筛选，不是 fixed-detector downstream result；未运行 DiffusionEdge/MSFI、`eval.py` 或 `show.py`。

## 4. Downstream 诊断事实

在历史 MyEdge 168 张带 GT split、fixed MSFI 50k 和 fixed DiffusionEdge baseline 50k 口径下，locked Stage1 `Final` 是 downstream negative control：它明显降低 ODS/OIS/AP/AC。后续主验证改用与增强指标一致的 MyEdge 166 complete-case。
...
当前 MyEdge 166 complete-case 是 downstream validation 主口径；502/496 只用于 Stage1 增强指标和 complete-case 对照；2770 full-pool 不能替代 166 downstream validation。
...
2770 不能写成正式 full-pool 结果，也不能替代 502/496 正式口径或 166 downstream validation。
...
- 502/496 增强指标已完成。
- locked Final 在历史 168 fixed-detector downstream 口径下是 negative control；后续主验证统一为 166 complete-case。
