"""White-balance-safe contrast branch implementation.

`RGHS` 是沿用的历史阶段名。当前主函数 `wb_safe_contrast` 的真实作用更接近
“白平衡安全对比增强分支”：以 Lab 亮度增强为主，结合平坦区抑制、色度保护
和后端 Lab 微调，生成供后续融合使用的增强分支。

从 2026-04-17 起，主实现文件名从 `RGHS.py` 调整为 `wb_safe_contrast.py`。
阶段名 `RGHS` 仍保留在结果目录、配置字段和实验记录里，用于兼容既有资产。
"""
...
def wb_safe_contrast(
    ...
    """White-balance-safe contrast enhancement branch.

    调参时可以先按五组理解：

    1. `strength`、`clip_limit`、`tile_grid`
       控制基础亮度增强强度，相当于这条分支的“主对比力度”。
    2. `p_low`、`p_high`、`pre_blur_sigma`
       控制进入局部增强前的亮度预拉伸与平滑，影响整体层次是否容易发硬。
    3. `deblock_*`、`flat_suppress`、`flat_floor`、`grad_sigma`
       控制块感、平坦区抑制和边缘/平坦区域的差异化增强。
    4. `chroma_preserve`、`adaptive_chroma`
       控制色度保护，是这条分支“不把白平衡重新拉坏”的关键。
    5. `brightness_boost`、`post_L_stretch`、`post_chroma_gain`
       控制后端的全局提亮和 Lab 微调，主要影响观感收口。
    """
